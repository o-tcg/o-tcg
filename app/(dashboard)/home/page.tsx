    ...
    <div className="mx-auto flex min-h-svh w-full max-w-7xl flex-col items-stretch gap-8 p-6">
      ...
      <nav className="flex w-full items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger>
              <Avatar className="h-10 w-10 cursor-pointer border-2 border-transparent transition-all hover:border-[#ff66aa]">
        ...
        <TabsContent value="packs" className="mt-8">
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {packs.map((pack) => (
              ...
              >
                <div className="relative flex aspect-3/4 flex-col items-center justify-center bg-zinc-900 p-6 text-center">
      ...
      <Dialog open={!!selectedPack} onOpenChange={() => setSelectedPack(null)}>
        <DialogContent className="border-zinc-800 bg-zinc-950 text-white outline-none sm:max-w-md">
          {selectedPack && (
            <PackOpener
              pack={selectedPack}
              userId={user.id}
